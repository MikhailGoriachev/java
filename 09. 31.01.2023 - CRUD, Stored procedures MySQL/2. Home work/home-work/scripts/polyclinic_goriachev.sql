...
INSERT INTO `appointments` VALUES (1,'2022-10-21',1,1),(2,'2022-10-24',2,1),(3,'2022-10-26',3,2),(4,'2022-10-28',4,3),(5,'2022-10-22',4,5),(6,'2022-10-11',3,5),(7,'2022-12-22',2,6),(8,'2022-10-13',5,2),(9,'2022-10-23',6,5),(10,'2022-10-15',9,6),(11,'2022-12-22',7,6),(12,'2022-10-28',3,7),(13,'2022-10-23',2,8),(14,'2022-10-28',4,9),(15,'2022-11-02',6,9);
...
INSERT INTO `doctors` VALUES (1,1,1,110,5),(2,2,2,600,3),(3,3,1,160,3.5),(4,4,3,250,3.2),(5,5,3,300,4.5),(6,6,2,600,4.5),(7,7,4,300,4),(8,8,5,250,3.5),(9,9,6,800,5.1),(10,10,6,1300,4.5),(11,11,2,350,3.1),(12,12,1,120,6.2);
...
/*!50001 VIEW `view_appointments` AS select `appointments`.`id` AS `id`,`appointments`.`appointment_date` AS `appointment_date`,`doc`.`surname` AS `doctor_surname`,`doc`.`name` AS `doctor_name`,`doc`.`patronymic` AS `doctor_patronymic`,`specialities`.`name` AS `speciality_name`,`doctors`.`price` AS `price`,`doctors`.`percent` AS `percent`,`pat`.`surname` AS `patient_surname`,`pat`.`name` AS `patient_name`,`pat`.`patronymic` AS `patient_patronymic`,`patients`.`born_date` AS `born_date`,`patients`.`address` AS `address`,`patients`.`passport` AS `passport` from ((`appointments` join ((`doctors` join `persons` `doc` on((`doctors`.`id_person` = `doc`.`id`))) join `specialities` on((`doctors`.`id_speciality` = `specialities`.`id`))) on((`appointments`.`id_doctor` = `doctors`.`id`))) join (`patients` join `persons` `pat` on((`patients`.`id_person` = `pat`.`id`))) on((`appointments`.`id_patient` = `patients`.`id`))) */;
...
-- Dump completed on 2023-02-01 23:53:23
