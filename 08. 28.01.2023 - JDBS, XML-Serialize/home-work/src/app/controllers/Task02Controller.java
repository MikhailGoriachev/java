...
public class Task02Controller implements IController {
    ...
    // название файла с данными статистики
    public final String STATISTIC_FILE_NAME = "app_data/statistic.bin";
    ...
    // для всех слов, из которых состоит текст определить: количество слов, суммарное количество букв в словах,
    // минимальная длина слова, максимальная длина слова, средняя длина слова
    public Number[] getStatistic() throws IOException {
        var words = new ArrayList<String>();
        ...
        try (var raf = new RandomAccessFile(STATISTIC_FILE_NAME, "rw")) {
            raf.writeLong(data[0].longValue());
            raf.writeLong(data[1].longValue());
            raf.writeLong(data[2].longValue());
            raf.writeDouble(data[3].longValue());
            raf.writeDouble(data[4].longValue());
        }
        
        try (var raf = new RandomAccessFile(STATISTIC_FILE_NAME, "r")) {            
            data[0] = raf.readLong();
            data[1] = raf.readLong();
            data[2] = raf.readLong();
            data[3] = raf.readDouble();
            data[4] = raf.readDouble();
        }
