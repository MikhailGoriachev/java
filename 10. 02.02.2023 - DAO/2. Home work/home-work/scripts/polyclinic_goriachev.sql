...
/*!50001 VIEW `view_appointments` AS select `appointments`.`id` AS `id`,`appointments`.`appointment_date` AS `appointment_date`,`doc`.`id` AS `id_doctor`,`doc`.`surname` AS `doctor_surname`,`doc`.`name` AS `doctor_name`,`doc`.`patronymic` AS `doctor_patronymic`,`specialities`.`id` AS `id_speciality`,`specialities`.`name` AS `speciality_name`,`doctors`.`price` AS `price`,`doctors`.`percent` AS `percent`,`pat`.`id` AS `id_patient`,`pat`.`surname` AS `patient_surname`,`pat`.`name` AS `patient_name`,`pat`.`patronymic` AS `patient_patronymic`,`patients`.`born_date` AS `born_date`,`patients`.`address` AS `address`,`patients`.`passport` AS `passport` from ((`appointments` join ((`doctors` join `persons` `doc` on((`doctors`.`id_person` = `doc`.`id`))) join `specialities` on((`doctors`.`id_speciality` = `specialities`.`id`))) on((`appointments`.`id_doctor` = `doctors`.`id`))) join (`patients` join `persons` `pat` on((`patients`.`id_person` = `pat`.`id`))) on((`appointments`.`id_patient` = `patients`.`id`))) */;
...
-- Dump completed on 2023-02-04  0:44:28
