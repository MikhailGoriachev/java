...
public class Task03Servlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        var data = TabletList.getTablets();

        if (req.getParameter("order-by") != null) {
            data = switch (req.getParameter("order-by")) {
                case "price" -> TabletList.orderByPriceDesc();
                case "type" -> TabletList.orderByType();
                case "manufacture" -> TabletList.orderByManufacture();
                default -> throw new IllegalStateException("Unexpected value: " + req.getParameter("orderBy"));
            };
        }

        req.setAttribute("tablets", data);
